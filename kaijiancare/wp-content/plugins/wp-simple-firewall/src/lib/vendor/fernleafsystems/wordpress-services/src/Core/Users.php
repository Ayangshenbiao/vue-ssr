...
class Users {
	...
	public function getCurrentUserLevel() {
		...
		return is_null( $oUser ) ? -1 : $oUser->get( 'user_level' );
	}
	...
	public function getCurrentWpUsername() {
		$oUser = $this->getCurrentWpUser();
		return is_null( $oUser ) ? '' : $oUser->get( 'user_login' );
	}
	...
	public function getUserByUsername( $sUsername ) {
		...
		if ( version_compare( Services::WpGeneral()->getVersion(), '2.8.0', '<' ) ) {
	...
	public function getUserById( $nId ) {
		if ( version_compare( Services::WpGeneral()
									  ->getVersion(), '2.8.0', '<' ) || !function_exists( 'get_user_by' ) ) {
			return null;
		}
	...
	public function isUserLoggedIn() {
		return function_exists( 'is_user_logged_in' ) ? is_user_logged_in() : null;
	}
